        ...
        return Object.fromEntries(Object.entries(nativeModuleToPatch).filter(([packagaeName, packageDetails]) => {
            ...
            if (fsExtra.existsSync(potentialBuHashFile)){
                ...
                if (lodash.isEqual(lodash.omit(buHashDetails, ["buHash"]), packageDetails)){
                    console.log(`The package '${packagaeName}' seems like it has already been patched for the runtime '${runtime}' (platform:- ${platform}, arch:- ${arch}). Skipping. If you think this is a mistake try delete and reinstalling the node module.`)
                    return false
                }
            }
        ...
        const _getNativeModules = async (_nodeModulesPath:string) => {
            ...
            for (const modulePath of await fsExtra.readdir(realNodeModulesPath)) {
                ...
                if (modulePath.startsWith('@')) {
                    await _getNativeModules(realPath);
                }
                if (await fsExtra.pathExists(path.resolve(realPath, 'node_modules'))) {
